...
function SharePage() {
    ...
    useEffect(() => {
        const detectEnvironmentAndFetchMessages = async () => {
            ...
            if (window.API_BASE_URL) {
                try {
                    const response = await axios.get(`${window.API_BASE_URL}/chat/share/${shareCode}`);
    ...
    useEffect(() => {
        if (chatWindowRef.current && messages.length > 0) {
            const firstMessage = chatWindowRef.current.querySelector('.share-page__message-container');
            if (firstMessage) {
        ...
        <div className="share-page">
            <div className="share-page__username-header">
                ...
                <div className="share-page__export-buttons">
                    <button className="share-page__button" onClick={handleExportAsImage} title="Export as Image">
                        <FaDownload/> Image
                    </button>
                    <button className="share-page__button" onClick={handleExportAsPDF} title="Export as PDF">
                        <FaDownload/> PDF
                    </button>
                </div>
            </div>
                    ...
                    messages.map((msg, index) => (
                        <div
                            ...
                            className={`share-page__message-container ${
                                msg.role === 'user' ? 'share-page__message--user' : 'share-page__message--assistant'
                            }`}
                        >
                            ...
                            <div
                                ...
                                onMouseEnter={() => {
                                    if (!msg.content.includes('```')) {
                                        const copyIcon = chatWindowRef.current.querySelectorAll('.copy-icon')[index];
                            ...
                            >
                                            ...
                                            return !inline ? (
                                                ...
                                                >
                                                    ...
                                                    <FaCopy className="copy-icon"
                                                            onClick={() => handleCopyText(String(children).trim())}/>
                                                </div>
                                            ) : (
                                ...
                                {!msg.content.includes('```') && (
                                    <FaCopy className="copy-icon" onClick={() => handleCopyText(msg.content)}/>
                                )}
                            </div>
                            <p className="share-page__timestamp">{new Date(msg.timestamp).toLocaleString()}</p>
                        </div>
                    ))
                ) : (
            ...
            {copyNotification && <div className="copy-notification">Copied to clipboard!</div>}
        </div>
    );
}
